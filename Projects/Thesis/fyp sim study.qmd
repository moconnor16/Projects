...
pred <- X_data %*% beta_true

y_data <- rbinom(n, size = 1, prob = probs(pred))
```

```{r}
df <- data.frame(y = y_data, x1 = x[, 1], x2 = x[, 2], x3 = x[, 3])
model <- glm(y ~ x1 + x2 + x3, data = df, family = binomial)

x1_seq <- seq(min(df$x1), max(df$x1), length.out = 100)

x2_fixed <- mean(df$x2)
x3_fixed <- mean(df$x3)

new_df <- data.frame(x1 = x1_seq, x2 = x2_fixed, x3 = x3_fixed)
new_df$prob <- predict(model, newdata = new_df, type = "response")

library(ggplot2)
ggplot() +
  geom_line(data = new_df, aes(x = x1, y = prob), color = "blue", size = 1.2) +
...
matplot(beta_history, type = "l", lty = 1, lwd = 2,
        ...
        cex.axis = 1,
        cex.main = 1.0)
abline(h = beta_true, col = 1:p, lty = 2)
```
...
```{r}
#Newton Raphson MSE
MSE_NR <- sum((beta_true - beta_NR)^2)
MSE_NR
```
...
for(i in 1:4) {
  ...
  abline(v = beta_true[i], col = "red", lwd = 2, lty = 2)
...
abline(h = beta_true, col = 1:4, lty = 2)
```
...
beta_init2 <- c(2, 2, 2, 2)
beta_init3 <- c(-2, -2, -2, -2)

mcmc1 <- MH_logistic(X, y, beta_init, iter = 10000, burnin = 1000, thin = 10, Sigma_prop = cov_mle)
...
chain1 <- as.mcmc(mcmc1)
chain2 <- as.mcmc(mcmc2)
chain3 <- as.mcmc(mcmc3)

chains_block <- mcmc.list(chain1, chain2, chain3)
gelman.plot(chains_block)
```
...
```{r}
#MCMC Block MSE
MSE_MCMC_block <- sum((beta_true - parameter_means)^2)
MSE_MCMC_block
```
...
mcmc_gibbs1 <- MHGibbs_logistic(X, y, beta_init, iter = 10000, burnin = 1000, thin = 10, Sigma_prop = cov_mle)
...
chain1 <- as.mcmc(mcmc_gibbs1)
chain2 <- as.mcmc(mcmc_gibbs2)
chain3 <- as.mcmc(mcmc_gibbs3)

chains_para <- mcmc.list(chain1, chain2, chain3)
gelman.plot(chains_para)
```
...
```{r}
#MCMC Parameter MSE
MSE_MCMC_para <- sum((beta_true - gibbs_parameter_means)^2)
MSE_MCMC_para
```
...
```{r}
#Laplace Approx MSE
MSE_laplace <- sum((beta_true - beta_MAP)^2)
MSE_laplace
```
...
```{r}
#Variational Approximation MSE
MSE_varapprox <- sum((beta_true - m)^2)
MSE_varapprox
```
...
results_table <- data.frame(
  ...
  MSE = c(MSE_NR, MSE_MCMC_block, MSE_MCMC_para, MSE_laplace, MSE_varapprox),
